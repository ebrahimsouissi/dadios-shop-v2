...
async function handleSales(body, env, request) {
  ...
  //   Whole body wrapped in try/catch so any throw surfaces as a
  //   structured 500 with err.message + err.stack (no more 1101 HTML).
  // ====================================================================
  if (action === 'admin_import_sale') {
    try {
      ...
      // Audit log is best-effort: a failure here shouldn't void the import.
      try {
      ...
      console.error('[admin_import_sale] crash:', err && err.message, err && err.stack);
      return json({
        ok: false,
        error: 'admin_import_sale crash: ' + (err && err.message),
        stack: err && err.stack,
      }, 500);
    }
  }
